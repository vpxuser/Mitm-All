...
func connect(conn net.Conn, addr string) error {
	return mitm.HeadProtocol(conn, addr)
	...
	//	return mitm.HttpsMITM(strings.Split(addr, ":")[0], addr, reader, client)
	//default:
...
//		var content string
//		if c, ok := protocol.ContentType[contentType]; !ok {
//			yaklog.Warnf(comm.SetColor(comm.RED_COLOR_TYPE, fmt.Sprintf("%s %s Unknown TLS Record Content Type : %v", Tag, connection, &protocol.TLSRecordLayer{ContentType: contentType, Version: version, Length: length})))
...
//			}
//		} else {
//			content = fmt.Sprintf("%s Content Type : %s ,", connection, comm.SetColor(comm.YELLOW_COLOR_TYPE, c))
//		}
...
//			if c, ok := protocol.MessageType[handshakeType]; !ok {
//				content = fmt.Sprintf("%s Message Type : %s", content, comm.SetColor(comm.RED_BG_COLOR_TYPE, comm.SetColor(comm.YELLOW_COLOR_TYPE, "Finished")))
//				yaklog.Infof("%s %s", Tag, content)
//			} else {
//				content = fmt.Sprintf("%s Message Type : %s", content, comm.SetColor(comm.RED_BG_COLOR_TYPE, comm.SetColor(comm.YELLOW_COLOR_TYPE, c)))
//				yaklog.Infof("%s %s", Tag, content)
//			}
//			// todo
//			if handshakeType == protocol.MessageTypeClientHello {
//				record, err := protocol.ParseTLSRecordLayer(append(recordHeader, recordPayload...))
//				if err != nil {
//					yaklog.Warnf("%s %v", Tag, err)
//				}
//				handshake, err := protocol.ParseHandshakeMessage(record.Fragment)
//				if err != nil {
//					yaklog.Warnf("%s %v", Tag, err)
//				}
//				clientHello, err := protocol.ParseClientHello(handshake.Data)
//				if err != nil {
//					yaklog.Warnf("%s %v", Tag, err)
//				}
//				sni, err := clientHello.GetServerNameIndication()
//				if err != nil {
//					yaklog.Warnf("%s %v", Tag, err)
//				}
//				if sni != nil {
//					hostName := sni.ServerNameList[0].HostName
//					yaklog.Debugf("%s Target Hostname : %s", Tag, comm.SetColor(comm.YELLOW_BG_COLOR_TYPE, comm.SetColor(comm.RED_COLOR_TYPE, hostName)))
//					//if err = mitm.HttpsMITM(hostName, Tag, reader, src); err != nil {
//					//	yaklog.Warnf(comm.SetColor(comm.RED_COLOR_TYPE, fmt.Sprintf("%s %v", Tag, err)))
//					//}
//				}
//			}
//		}
