...
func ParseHandshake(data []byte, ctx *context.Context) (*Handshake, error) {
	if len(data) < 4 {
	...
	default:
		yaklog.Warnf(colorutils.SetColor(colorutils.MAGENTA_COLOR_TYPE, fmt.Sprintf("not support Handshake Type : %d", handshake.HandshakeType)))
	}
...
func (h *Handshake) GetRaw() []byte {
	...
	if h.Payload == nil {
		...
		default:
			yaklog.Warnf(colorutils.SetColor(colorutils.MAGENTA_COLOR_TYPE, fmt.Sprintf("not support Handshake Type : %d", h.HandshakeType)))
		}
	}
	return append(handshake, h.Payload...)
}
